        ...
        <div class="card-body">
            ...
            @endif
                ...
                @foreach($orderFormData as $data)
                    @if(!in_array($data['field_id'], $excludedFields))
                                 ...
                                 <div class="col-sm-9">
                                     @switch($data['field_type'])
                                         @case('id')
                                             ...
                                             @break
                                         @case('amount to warehouse')
                                             <input disabled type="text" class="form-control" id="{{ $data['field_id'] }}" value="{{ $data['value'] }}">
                                             <small id="{{ $data['field_id'] }}" class="form-text text-muted">This is autogenerated field and can not be edited</small>
                                             @break
                                         @case('available amount from warehouse')
                                             <input disabled type="text" class="form-control" id="{{ $data['field_id'] }}" value="{{ $data['value'] }}">
                                         ...
                                         @case('select glue')
                                             @if($itemFromWarehouse)
                                                 <select disabled class="form-control" id="{{ $data['field_id'] }}">
                                                     @foreach($data['input_select'] as $option)
                                                         <option value="{{ $option }}" {{ $data['value'] == $option ? ' selected' : '' }}> {{ $option }}</option>
                                                     @endforeach
                                                 </select>
                                                 <small class="form-text text-muted">Field updates not allowed because item taken from warehouse</small>
                                             @else
                                                 <select class="form-control" id="{{ $data['field_id'] }}" name="field_{{$data['field_id'] }}">
                                                     @foreach($data['input_select'] as $option)
                                                         <option value="{{ $option }}" {{ $data['value'] == $option ? ' selected' : '' }}> {{ $option }}</option>
                                         ...
                                         @case('select warehouse')
                                             @if($itemFromWarehouse)
                                                 <select disabled class="form-control" id="{{ $data['field_id'] }}">
                                                     ...
                                                     @foreach($data['input_select'] as $id => $name)
                                                         <option value="{{ $id }}" {{ $data['value'] == $id ? ' selected' : '' }}> {{ $name }}</option>
                                                     @endforeach
                                                 </select>
                                                 <small class="form-text text-muted">Field updates not allowed because item taken from warehouse</small>
                                             @else
                                         ...
                                         @case('select transport')
                                             ...
                                             @break
                                         @case('dynamic select')
                                             @if($itemFromWarehouse && in_array($data['field_id'], $lockedFields))
                                                 <select disabled class="form-control select-with-search" id="{{ $data['field_id'] }}">
                                                     <option>-</option>
                                                     @foreach($data['input_select'] as $option)
                                                         <option value="{{ $option }}" {{ $data['value'] == $option ? ' selected' : '' }}> {{ $option }}</option>
                                                     @endforeach
